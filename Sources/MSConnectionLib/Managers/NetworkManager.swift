...
public actor NetworkManager {
    ...
    public func getResponseWithData<T: Decodable, U: Encodable>(
        from url: String,
        lang: String = "en",
        parameters: U? = nil,
        responseType: T.Type,
    ...
    ) async -> (Result<T, MultipleDecodingErrors> , Data?) {
        ...
        do {
            ...
            return (.failure(MultipleDecodingErrors(errors: decodingErrors)) , nil)
        } catch {
            // Handle other errors
            logError(error)
            return (.failure(MultipleDecodingErrors(errors: [.other(error)])) , nil)
        }
    }
    ...
    public func post<T: Decodable, U: Encodable>(
        ...
        do {
            ...
            if let httpResponse = urlResponse as? HTTPURLResponse {
                ...
                guard (200...404).contains(statusCode) else {
